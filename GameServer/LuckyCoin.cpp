...
void CLuckyCoin::DGLuckyCoinExchangeRecv(SDHP_LUCKY_COIN_EXCHANGE_RECV* lpMsg) // OK
{
	...
	{
	case 10:
		if(gItemBagManager.GetItemBySpecialValue(ITEM_BAG_SPECIAL_LUCKY_COIN,0,-1,lpObj,&item) == 0)
		{
			pMsg.result = 4;
			DataSend(lpObj->Index,(BYTE*)&pMsg,pMsg.header.size);
			return;
		...
		break;
	case 20:
		if(gItemBagManager.GetItemBySpecialValue(ITEM_BAG_SPECIAL_LUCKY_COIN,1,-1,lpObj,&item) == 0)
		{
			pMsg.result = 4;
			DataSend(lpObj->Index,(BYTE*)&pMsg,pMsg.header.size);
			return;
		...
		break;
	case 30:
		if(gItemBagManager.GetItemBySpecialValue(ITEM_BAG_SPECIAL_LUCKY_COIN,2,-1,lpObj,&item) == 0)
		{
			pMsg.result = 4;
			DataSend(lpObj->Index,(BYTE*)&pMsg,pMsg.header.size);
			return;
