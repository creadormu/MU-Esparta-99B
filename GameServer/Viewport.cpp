...
void CViewport::DestroyViewportPlayer1(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if(lpTarget->Connected != OBJECT_ONLINE || lpTarget->Teleport != TELEPORT_NONE)
		{
			lpObj->VpPlayer[n].state = VIEWPORT_DESTROY;
			continue;
		}
...
void CViewport::GCViewportPlayerSend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if(lpTarget->State == OBJECT_CREATE && lpTarget->Teleport == TELEPORT_NONE)
		{
			info.index[0] |= 0x80;
		}
...
void CViewport::GCViewportMonsterSend(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if(lpTarget->State == OBJECT_CREATE)
		{
			...
			if(lpTarget->Teleport != TELEPORT_NONE)
			{
				info.index[0] |= 0x40;
			}
		}
...
void CViewport::GCViewportSimplePlayerSend(LPOBJ lpObj) // OK
{
	...
	if(lpObj->State == OBJECT_CREATE && lpObj->Teleport == TELEPORT_NONE)
	{
		info.index[0] |= 0x80;
	}
