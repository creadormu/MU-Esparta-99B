...
void CObjectManager::ObjectSetStateCreate(int aIndex) // OK
{
	...
	if(lpObj->Type == OBJECT_USER && lpObj->Teleport == TELEPORT_READY)
	{
		lpObj->Teleport = TELEPORT_UNK;
		...
		lpObj->Teleport = TELEPORT_NONE;
		lpObj->PathCur = 0;
		lpObj->PathCount = 0;
		lpObj->PathStartEnd = 0;
	}

	if(lpObj->Type == OBJECT_MONSTER && lpObj->Teleport == TELEPORT_READY)
	{
		lpObj->Teleport = TELEPORT_UNK;
	...
	if(lpObj->Type == OBJECT_NPC && lpObj->Teleport == TELEPORT_READY)
	{
		lpObj->Teleport = TELEPORT_UNK;
	...
	lpObj->Teleport = ((lpObj->Teleport==TELEPORT_STANDBY)?((lpObj->DieRegen==0)?(((GetTickCount()-lpObj->TeleportTime)>100)?TELEPORT_READY:lpObj->Teleport):TELEPORT_NONE):lpObj->Teleport);
...
void CObjectManager::ObjectSetStateProc() // OK
{
	for(int n=0;n < MAX_OBJECT;n++)
	{
		...
		lpObj->Teleport = ((lpObj->Teleport==TELEPORT_UNK)?TELEPORT_NONE:lpObj->Teleport);
...
void CObjectManager::CharacterUpdateMapEffect(LPOBJ lpObj) // OK
{
	...
	for(int i=0;i < MAX_EFFECT_LIST;i++)
	{
		...
		gEffectManager.GCEffectStateSend(lpObj,1,lpEffect->m_index);
	}
...
void CObjectManager::CharacterLifeCheck(LPOBJ lpObj,LPOBJ lpTarget,int damage,int DamageType,int flag,int type,int skill,int ShieldDamage,bool combo) // OK
{
	...
	if(lpTarget->Life <= 0 && lpTarget->Live != 0)
	{
		...
		lpTarget->Teleport = ((lpTarget->Teleport==TELEPORT_STANDBY)?TELEPORT_NONE:lpTarget->Teleport);
