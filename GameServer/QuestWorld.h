...
public:
	...
	int GetStateByIndex(int QuestIndex,int QuestGroup);
	bool AddQuestWorldList(LPOBJ lpObj,int QuestIndex,int QuestGroup,int QuestNumber,int QuestState,int Start);
	...
	bool CharacterUseLetter(LPOBJ lpObj);
...
private:
	std::vector<QUEST_WORLD_INFO> m_QuestWorldInfo;
};
