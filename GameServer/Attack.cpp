...
bool CAttack::Attack(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,int damage,int count,bool combo) // OK
{
	...
	if(lpObj->Map != lpTarget->Map || (gServerInfo.m_TeleportAttackCheck != 0 && lpObj->Teleport == TELEPORT_READY))
	{
		return 0;
	}
	...
	if(lpTarget->Type == OBJECT_NPC || lpTarget->Live == 0 || lpTarget->State != OBJECT_PLAYING || lpTarget->Teleport != TELEPORT_NONE)
	{
		return 0;
	}
	...
	if(lpObj->Type == OBJECT_USER)
	{
		if(lpSkill == 0)
		{
			if(lpObj->Inventory[INVENTORY_SLOT_WEAPON1].m_Index == GET_ITEM(2,4) || lpObj->Inventory[INVENTORY_SLOT_WEAPON1].m_Index == GET_ITEM(2,5))
			{
				if(gObjCheckResistance(lpTarget,0) == 0)
				{
					if(send != 0)
					{
						if((GetLargeRand()%100) < 50)
						{
							gEffectManager.AddEffect(lpTarget,0,gSkillManager.GetSkillEffect(SKILL_ICE),10,0,0,0,0);

							gSkillManager.GCSkillAttackSend(lpObj,SKILL_ICE,lpTarget->Index,1);
						}
					}
				}
			}
...
bool CAttack::CheckPlayerTarget(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(gGuild.gObjTargetGuildWarCheck(lpObj,lpTarget) == 0)
	{
		if(lpTarget->Guild != 0 && lpTarget->Guild->WarState != 0)
		{
			...
			#else
			if(gCustomArena.CheckMap(lpTarget->Map) == 0 && CC_MAP_RANGE(lpTarget->Map) == 0 && gDuel.CheckDuel(lpObj,lpTarget) == 0 && gGensSystem.CheckGens(lpObj,lpTarget) == 0)
			#endif
			{
				return 0;
			}
