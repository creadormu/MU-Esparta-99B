...
bool CSkillManager::CheckSkillTarget(LPOBJ lpObj,int aIndex,int bIndex,int type) // OK
{
	...
	if(gObj[aIndex].Live == 0 || gObj[aIndex].State != OBJECT_PLAYING || gObj[aIndex].Teleport != TELEPORT_NONE)
	{
		return 0;
	}
...
bool CSkillManager::SkillStern(int aIndex,int bIndex,CSkill* lpSkill,BYTE angle) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if(gSkillHitBox.HitCheck(nangle,lpObj->X,lpObj->Y,gObj[index].X,gObj[index].Y) != 0)
		{
			if(gObj[index].Teleport == TELEPORT_NONE && gEffectManager.CheckEffect(&gObj[index],EFFECT_IRON_DEFENSE) == 0 && gEffectManager.CheckEffect(&gObj[index],EFFECT_IRON_DEFENSE_IMPROVED) == 0)
			{
