...
void CDevilSquare::GiveUserRewardExperience(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	for(int n=0;n < MAX_DS_USER;n++)
	{
		...
		gQuestWorldObjective.QuestWorldObjectiveSpecial(&gObj[lpLevel->User[n].Index],QUEST_WORLD_OBJECTIVE_DEVIL_SQUARE_CLEAR,lpLevel->Level+1);
...
void CDevilSquare::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	gQuestWorldObjective.QuestWorldObjectiveSpecial(lpTarget,QUEST_WORLD_OBJECTIVE_DEVIL_SQUARE_POINTS,level+1);
}
